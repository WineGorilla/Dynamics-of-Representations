...
#CUDA_VISIBLE_DEVICES=1 python analysis/pearson/pearson_a.py
STANDARD_N_ROI = 200
...
def apply_hrf_to_embedding(emb, tr=2.0):
    """
    emb: (T, D)
    使用 FFT 一次性对所有维度做 HRF 卷积，避免逐维 for 循环
    ...
    E = np.fft.rfft(emb, n=n, axis=0)        # (n//2+1, D)
    H = np.fft.rfft(hrf, n=n)                 # (n//2+1,)
    convolved = np.fft.irfft(E * H[:, None], n=n, axis=0)  # (n, D)

    return convolved[:T].astype(emb.dtype)
...
def run_one_model_fused(args):
    model_name, center, alpha = args
    ...
    if pearson_list:
        ...
        print(f"[DONE] {model_name} center={center}, alpha={alpha}, shape={group_mean.shape}")
    else:
        print(f"[SKIP] {model_name} center={center}")

    return model_name, center
...
if __name__ == "__main__":
    ...
    # 把 center × model 全部展开成一个任务列表，共用一个进程池
    all_tasks = [
        (model, center, ALPHA)
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(run_one_model_fused, task): task
        ...
        for f in as_completed(futures):
            ...
            try:
                ...
                print(f"[ERROR] model={task[0]} center={task[1]}: {e}")
...
def run_one_lang_model_fused(model_name, center, alpha=50.0):

    center_str = f"{int(center*10):02d}"

    FMRI_ROOT = "data/lang/fmri"
    EMB_DIR   = f"data/lang/design_matrix_dmd_soft{center_str}/{model_name}"
    SAVE_ROOT = f"results/pearson/{center_str}/lang_fused/{model_name}"
    ...
    for subj in subs:
        ...
        # ===== concat runs for this subject =====
        if len(X_runs) == 0:
            continue

        X_subj = np.concatenate(X_runs, axis=0)
        Y_subj = np.concatenate(Y_runs, axis=0)

        pearson = fused_roi_pearson(X_subj, Y_subj, alpha=alpha)

        subject_scores.append(pearson)

    # ===== group mean =====
    if subject_scores:

        group_mean = np.mean(
            np.stack(subject_scores, axis=0),
            axis=0
        )
...
if __name__ == "__main__":
    ...
    for center in centers:
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {
                executor.submit(run_one_lang_model_fused, m,center, ALPHA): m
